...
function Bundle.install(mod, spec, features)
  ...
  -- A Gen 2 cart's save used to be scoped from here.  `src/core/gen2/Save.lua`
  -- named its file out of the VERSION alone, so a cart on Gold, Silver or
  -- Crystal read and wrote the base game's playthrough and registered its slot
  -- in the base game's registry on the way; runtime/cartsave2.lua rewrote the
  -- paths on their way to disk.
  --
  -- The ENGINE does it now, and to the same filenames -- `save_cart_<id>.lua`
  -- flat, `saves/cart_<id>/<slot>.lua` with a slot -- so nothing moved on
  -- anybody's disk when the shim came out.  This bundle's floor is the release
  -- that carries it (manifest.json `game_version`), which is what makes
  -- deleting the shim safe rather than merely tidy: there is no build this can
  -- install on where the engine is not already doing the job.
  --
  -- tests/cartsave2_test.lua is the guard that it keeps doing it.
