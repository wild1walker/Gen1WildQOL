-- Gen1Recomp 0.1.86+ sandbox bootstrap for Gen1Follower.
--
...
return function(mod)
  ...
  -- facade. Current Gen 1 keeps that predicate private, which older builds
  ...
  if generation == 1 and type(PikachuFollower.setShouldSpawn) ~= "function" then
    ...
    if type(shim) ~= "table" then
      ...
      -- private Yellow/Pikachu spawn gate is too narrow for this mod. Satisfy
      ...
      local function withNativeSpawnGate(game, fn, ...)
        ...
        -- follower this mod selected is a different, healthy mon -- and no
  ...
  local chunk, compileErr = load(source, "@Gen1Follower/main.lua")
  if not chunk then
    if mod.log and mod.log.error then
  ...
  if mod.exports then
    mod.exports.providerRepository = "wild1walker/Gen1Follower"
    -- Consumers written against the PokePC Followers provider contract may
    -- compare providerRepository literally. Publish the lineage separately
    -- so those checks can recognise this build without mislabelling it.
    mod.exports.upstreamRepository = "mfrtechconsult/PokePCFollowers"
  ...
  -- exactly like vanilla until Gen1Follower is initialized again.
  if sandboxShim then
